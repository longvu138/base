...
export const buildShipmentListParams = ({
  page,
  pageSize,
  filters,
...
export const normalizeShipmentListResponse = (
  response: any,
  fallbackPage = 1,
  fallbackPageSize = 20,
) => {
  ...
  const pageSize = Number(
    headers["x-page-size"] ?? body?.pageSize ?? body?.size ?? fallbackPageSize,
  );
  const total = Number(
    headers["x-total-count"] ?? body?.total ?? body?.totalElements ?? data.length,
  );
...
export const useInfiniteShipmentsQuery = (
  params: Record<string, any>,
  pageSize = 20,
) =>
  useInfiniteQuery({
    queryKey: ["shipments.list", "mobile", params],
    queryFn: async ({ pageParam = 1 }) => {
      const response = await ShipmentApi.getShipments({
        ...params,
        page: Math.max(Number(pageParam) - 1, 0),
        size: pageSize,
      });
      return normalizeShipmentListResponse(response, Number(pageParam), pageSize);
    },
    initialPageParam: 1,
    getNextPageParam: (lastPage, allPages) => {
      const loaded = allPages.reduce(
        (count, page) => count + (Array.isArray(page.data) ? page.data.length : 0),
        0,
      );

      if (loaded >= Number(lastPage.total || 0)) return undefined;
      if (lastPage.totalPage && lastPage.current >= lastPage.totalPage) {
        return undefined;
      }
...
export const useShipmentsLogic = ({
  ...
  const apiParams = useMemo(
    () => buildShipmentListParams({ page, pageSize, filters }),
...
export const useMobileShipmentsLogic = ({
  pageSize,
  filters,
}: {
  pageSize: number;
  ...
  const apiParams = useMemo(
    () => buildShipmentListParams({ page: 1, pageSize, filters }),
    [pageSize, filters],
  );
  const shipmentQuery = useInfiniteShipmentsQuery(apiParams, pageSize);
  const { data: statusData } = useShipmentStatusesQuery();
  const { data: statisticData } = useShipmentStatisticQuery();
  const { data: servicesData, isLoading: isServicesLoading } =
    useShipmentServicesQuery();
  ...
  const shipmentData = useMemo(() => {
    const pages = shipmentQuery.data?.pages || [];
    ...
    return {
      ...
      totalPage: lastPage?.totalPage || 1,
    };
  }, [pageSize, shipmentQuery.data?.pages]);
  ...
  return {
    shipmentData,
    isShipmentLoading: shipmentQuery.isLoading,
    isShipmentFetching: shipmentQuery.isFetching,
    isFetchingNextPage: shipmentQuery.isFetchingNextPage,
    hasNextPage: shipmentQuery.hasNextPage,
    fetchNextPage: shipmentQuery.fetchNextPage,
    statusData,
    statisticData,
    servicesData,
    serviceGroupsData,
...
export const useMobileShipmentsPage = () => {
  const { t } = useTranslation();
  const { notification } = App.useApp();
  const [form] = Form.useForm();
  const [expanded, setExpanded] = useState(false);
  ...
  const { applyFilters, clearFilters, filters } = useFilterWithURL({ form });
  const logic = useMobileShipmentsLogic({ pageSize, filters });
  const exportMutation = useExportShipmentsMutation();
  const importMutation = useImportShipmentsMutation();
