...
const normalizeWithdrawalSlipFilters = (filters: Record<string, any>) => {
    ...
    if (params.createdAtRange) {
        params.createdAtFrom = params.createdAtRange[0]?.startOf?.('day')?.toISOString?.() || params.createdAtRange[0]?.toISOString?.();
        params.createdAtTo = params.createdAtRange[1]?.endOf?.('day')?.toISOString?.() || params.createdAtRange[1]?.toISOString?.();
...
const buildWithdrawalSlipPayload = (values: any, banksData: any[] = []) => {
    ...
    return {
        ...
        beneficiaryBankName: bankCode === 'other' ? values.beneficiaryBankName : bank?.name,
...
const normalizeLogItems = (items: any[] = [], statusData: any[] = []) =>
    items.map((item) => {
        ...
        const oldStatus = statusData.find((status: any) => status.code === oldStatusCode);
        const newStatus = statusData.find((status: any) => status.code === newStatusCode);
...
const useWithdrawalSlipActions = (banksData: any[] = [], statusData: any[] = []) => {
    ...
    const submitCreateSlip = async () => {
        ...
        await createMutation.mutateAsync(buildWithdrawalSlipPayload(values, banksData));
