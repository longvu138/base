import {
  useStatisticSummaryQuery,
  type StatisticSummary,
  type StatisticType,
} from "../useStatisticHooks";
...
export const useStatisticsPage = () => {
  ...
  const tabs = useMemo<StatisticsTab[]>(() => {
    ...
    const enableRequestPayment =
      tenantConfig.peerPaymentConfig?.enabled === true;

    return [
      ...(enableOrder
        ? [{ key: "order" as const, labelKey: "statistic.order" }]
        : []),
      ...(enableShipment
        ? [{ key: "shipment" as const, labelKey: "statistic.shipment" }]
        : []),
      ...(enableRequestPayment
        ? [
            {
              key: "request_payment" as const,
              labelKey: "statistic.request_payment",
            },
          ]
        : []),
    ];
...
export const useStatisticDetailPage = (
  type: StatisticType,
  tenantCode?: string,
) => {
  const { t } = useTranslation();
  ...
  return {
    ...
    month: {
      title: t("statistic.title", {
        date: formatStatisticDate(now, monthFormat),
      }),
    ...
    year: {
      title: t("statistic.title", {
        date: formatStatisticDate(now, yearFormat),
      }),
