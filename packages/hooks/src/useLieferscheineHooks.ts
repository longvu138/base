...
export const useLieferscheineInfiniteQuery = (params: any) => {
  return useInfiniteQuery({
    queryKey: ["lieferscheine.list.infinite", params],
    initialPageParam: 0,
    queryFn: async ({ pageParam = 0 }) => {
      const res = await LieferscheineApi.getLieferscheine({
        ...params,
...
export const useLieferscheinePackagesQuery = (code?: string) => {
  return useQuery({
    ...
    queryFn: async () => {
      const res = await LieferscheineApi.getLieferscheinePackages(
        code as string
      );
      return res.data || [];
    },
...
export const useLieferscheineDeliveriesQuery = (code?: string) => {
  return useQuery({
    ...
    queryFn: async () => {
      const res = await LieferscheineApi.getLieferscheineDeliveries(
        code as string
      );
      return res.data || [];
    },
