...
export const DeliveryRequestApi = {
  getAvailableOrders: () => {
    return ApiClient.auth.get(
      `customer/delivery_requests/available_orders?size=9999`,
    );
  },
  getShippingMethods: () => {
  ...
  getDeliveryRequestPackages: (code: string) => {
    return ApiClient.auth.get(`customer/delivery_requests/${code}/packages`);
  },
  getDeliveryRequestStatuses: () => {
    return ApiClient.auth.get(
      `categories/delivery_request_statuses?size=1000&sort=position:asc`,
    );
  },
};
