...
- Backend fallback `variantCode` về `default` nếu tenant có variant sai/không hỗ trợ.
...
|---|---|
| Tenant | `baogam` |
| Variant/UI | `default` / `StyleDefault` |
...
- `VARIANT_NAMES`: `default`, `thanhla`, `gobiz`.
...
|---|---|---|
| `baogam` | `default` | giao diện phổ thông |
| `thien_long` | `default` | cùng variant với Baogam, khác màu |
| `free_sample_3` | `default` | sample free tenant |
| `gobiz` | `gobiz` | giao diện premium/specialized |
| `tetetete` | `thanhla` | giao diện hiện đại |
| `thanhla` | `thanhla` | giao diện Thanhla |
...
Nếu `tenantConfig` chưa có, `variantCode` mặc định là `default`.

Ví dụ mapping:

| Variant | Page key | Component |
|---|---|---|
| `default` | `orders` | `OrdersStyleDefault` |
| `thanhla` | `login` | `LoginStyleThanhla` |
| `gobiz` | `claims` | `ClaimsStyleGobiz` |
...
- nhiều page dùng `StyleThanhla`
- shipments dùng `Shipments`
- menu preset `base`

Lưu ý theo source hiện tại: một số tên override `thanhla` không khớp với file Web thật, nên `DynamicVariant` sẽ fallback theo `fallbackName` của page:

- `deliveryNotes` map `DeliveryNotesStyleThanhla`, nhưng Web có `DeliveryNoteStyleThanhla.tsx`.
- `packages` map `PackagesStyleThanhla`, nhưng Web có `PackageStyleThanhla.tsx`.
- `withdrawalSlips` map `WithdrawalSlipsStyleThanhla`, nhưng Web có `WithdrawalSlipStyleThanhla.tsx`.
...
Nếu variant code không nằm trong `VARIANT_DEFAULTS`, `getVariantDefaults()` trả default chung. Tuy nhiên backend hiện đã chặn variant sai và trả `default`.
...
Fallback layout là `LayoutStyleDefault`.
...
1. Tạo folder page.
2. Tạo ít nhất một component fallback, thường là `PageStyleDefault.tsx`.
...
```txt
[useVariant] orders -> OrdersStyleDefault (code: default)
```
