...
import { SearchOutlined, RedoOutlined, ArrowRightOutlined, BoxPlotOutlined } from '@ant-design/icons';
...
export const OrdersStyle3: React.FC<{ isTabView?: boolean }> = ({ isTabView }) => {
    ...
    const getStatusName = (code: string) => {
        return statusData?.find((s: any) => s.code === code)?.name || code;
    };
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (status: string) => (
                <Tag color={getStatusColor(status)} className="rounded-lg px-3 py-0.5 m-0 border-0 font-bold text-[10px] uppercase shadow-sm">
                ...
                <div className="flex justify-end mb-4">
                    <div className="flex flex-wrap gap-3">
                        <AntInput
                            ...
                            onChange={(e) => setSearchText(e.target.value)}
                            onPressEnter={handleSearch}
                            className="w-full md:w-80 h-10 rounded-xl"
                        />
                        <AntButton
                            type="primary"
                            icon={<SearchOutlined />}
