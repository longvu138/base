...
import { Form, Input as AntInput, Button as AntButton, Tag, Skeleton as AntSkeleton, Empty, DatePicker } from 'antd';
import { Pagination } from '@repo/ui';
import { useTranslation } from '@repo/i18n';
import { SearchOutlined, RedoOutlined, ArrowRightOutlined, RocketOutlined, FilterOutlined } from '@ant-design/icons';
import './ShipmentsStyleGobiz.css';

const { RangePicker } = DatePicker;

interface ShipmentsStyleGobizProps {
    data: any;
    isLoading: boolean;
    statusData?: any[];
    statusOptions: any[];
...
export const ShipmentsStyleGobiz: React.FC<ShipmentsStyleGobizProps> = ({
    data, isLoading, statusData, statusOptions,
    page, pageSize, setPage, setPageSize,
    form, applyFilters, handleReset, filters, isTabView
}) => {
    useTranslation();
    ...
    return (
        <div className={`shipments-style-3-wrapper space-y-6 ${isTabView ? 'p-4' : ''}`}>
            ...
            {!isTabView && (
                <div className="bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 p-5 rounded-[2rem] shadow-xl shadow-primary/5 border border-white dark:border-gray-800">
                    <div className="mb-5">
                        <h1 className="text-2xl font-black tracking-tight text-gray-900 dark:text-white m-0">Ký gửi <span className="text-primary">Gobiz</span></h1>
                        <p className="text-gray-400 text-xs m-0 font-medium">Hành trình hàng hóa minh bạch.</p>
                    </div>
                    {searchBar()}
                </div>
            )}
                ...
                <div className="bg-white dark:bg-gray-800 p-5 rounded-[2rem] shadow-lg border border-gray-100 dark:border-gray-700 space-y-5">
                    <Form form={form} layout="vertical" onValuesChange={() => applyFilters(form.getFieldsValue())}>
                        <div className="grid grid-cols-1 gap-4">
                            <Form.Item name="code" label={<span className="text-[10px] font-black uppercase text-primary tracking-widest pl-1">Mã yêu cầu</span>} className="mb-0">
                                <AntInput placeholder="Ví dụ: SHP123" className={inputCls} allowClear />
                            </Form.Item>
                            <Form.Item name="waybillCode" label={<span className="text-[10px] font-black uppercase text-primary tracking-widest pl-1">Mã vận đơn</span>} className="mb-0">
            ...
            <div className="space-y-4 min-h-[400px]">
                        ...
                        >
                            ...
                            <div className="grid grid-cols-2 gap-4 mb-4 relative z-10">
                                <div className="bg-gray-50 dark:bg-gray-900/50 p-3 rounded-2xl border border-gray-100 dark:border-gray-700">
                                    <div className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">Mã vận đơn</div>
                                    <div className="text-xs font-black text-gray-700 dark:text-gray-200 truncate">{record.waybillCode || '—'}</div>
                                </div>
                                <div className="bg-gray-50 dark:bg-gray-900/50 p-3 rounded-2xl border border-gray-100 dark:border-gray-700">
                                    <div className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">Cân nặng</div>
                                    <div className="text-xs font-black text-gray-700 dark:text-gray-200">{record.weight || 0} kg</div>
                                </div>
                            </div>
                ...
                {(!data?.data || data.data.length === 0) && !isLoading && (
                    <div className="bg-white dark:bg-gray-800 rounded-[2rem] p-12 text-center border border-dashed border-gray-200 dark:border-gray-700">
                        <Empty description={<span className="text-gray-400 font-bold">Không có yêu cầu ký gửi</span>} />
                    </div>
                )}
            </div>
