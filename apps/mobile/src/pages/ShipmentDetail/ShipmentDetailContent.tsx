    ...
    <Space direction="vertical" size="middle" style={styles.fullWidth}>
      ...
      <Card>
        ...
        >
          ...
          {(statusInfo?.confirmable || statusInfo?.cancellable) && (
            <Flex
              justify="start"
              gap={8}
              wrap="wrap"
          ...
          items={[
            {
              ...
              children: (
                <Spin spinning={isProductsLoading}>
                  <Space direction="vertical" size={token.marginSM} style={styles.fullWidth}>
                        ...
                        >
                          <Space direction="vertical" size={token.marginSM} style={styles.fullWidth}>
                            <Flex justify="space-between" align="flex-start" gap={token.marginSM}>
                              <Flex align="flex-start" gap={token.marginSM} style={{ minWidth: 0 }}>
                                {item.productImage ? (
                                  <Avatar
                                    shape="square"
                                    size={52}
                              ...
                              {statusInfo?.productUpdatable && (
                                <Flex justify="end" align="center" style={{ flex: "none" }}>
                                  <Button
                                    type="link"
                            ...
                            <Space direction="vertical" size={2} style={styles.fullWidth}>
                              {getVariantText(item) && (
                                <Text type="secondary" style={{ wordBreak: "break-word" }}>
                                  {getVariantText(item)}
                                </Text>
                              ...
                              <Text type="secondary" style={{ wordBreak: "break-word" }}>
                                <ShopOutlined style={{ marginRight: token.marginXXS }} />
                                {item.merchantName || empty}
                              </Text>
                              <Text type="secondary" style={{ wordBreak: "break-word" }}>
                                <EnvironmentOutlined style={{ marginRight: token.marginXXS }} />
                            ...
                            <Row gutter={[token.marginSM, token.marginSM]}>
                              <Col xs={24} md={8}>
                                <Card size="small" styles={{ body: { padding: token.paddingSM } }}>
                                  <Space direction="vertical" size={2}>
                                    <Text type="secondary">{t("shipments.hsCode")}</Text>
                                    <Text strong>
                                      {display(
                                        asArray(hsList).find(
                              ...
                              <Col xs={12} md={8}>
                                <Card size="small" styles={{ body: { padding: token.paddingSM } }}>
                                  <Space direction="vertical" size={2}>
                                    <Text type="secondary">{t("order.quantity")}</Text>
                              ...
                              <Col xs={12} md={8}>
                                <Card size="small" styles={{ body: { padding: token.paddingSM } }}>
                                  <Space direction="vertical" size={2}>
                                    <Text type="secondary">{t("order.sale_price")}</Text>
                                    <Text strong>
              ...
              children: (
                <Spin spinning={isWaybillsLoading || isParcelsLoading}>
                  <Space direction="vertical" size={token.marginSM} style={styles.fullWidth}>
                    ...
                    >
                      <Text>
                        Mã vận đơn ({waybillRows.filter((item) => item.code).length}/
                        {shipmentWaybillThreshold})
                      </Text>
                    </Card>

                    {waybillRows.length === 0 ? (
                      <Empty description="Chưa có mã vận đơn" />
                    ) : (
                      sortNewest(waybillRows).map((row, index) => {
                        const hasCreateRow =
                          row.code === null && waybillRows.some((item) => item.code === null);
                        const rowParcels = getParcelsForWaybill(row, parcelRows);
                        const sortedParcels = sortNewest(rowParcels, "modifiedAt");
                        const expanded = row.code !== null && expandedWaybillKeys.includes(row.code);

                        if (hasCreateRow) {
                          return (
                            <Card
                              key={`waybill-create-${index}`}
                              size="small"
                              style={{
                                borderRadius: 0,
                                background: token.colorFillAlter,
                            ...
                            >
                              {isCreatingWaybill ? (
                                <Flex wrap="wrap" align="center" gap={token.marginSM}>
                                  <Input
                          ...
                          >
                            <Space direction="vertical" size={token.marginSM} style={styles.fullWidth}>
                              <Flex justify="space-between" align="flex-start" gap={token.marginSM}>
                                ...
                                <Flex align="center" gap={token.marginXS} style={{ flex: "none" }}>
                                  {statusInfo?.updatable &&
                                    row.customer &&
                                    rowParcels.length === 0 && (
                                      <Popconfirm
                                  ...
                                  <Button
                                    type="text"
                                    icon={expanded ? <ShrinkOutlined /> : <DownOutlined />}
                                    onClick={() =>
                                      setExpandedWaybillKeys(expanded || row.code === null ? [] : [row.code])
                                    }
                                  />
                                </Flex>
                              </Flex>
                              ...
                              <Row gutter={[token.marginSM, token.marginSM]}>
                                <Col xs={12}>
                                  <Card size="small" styles={{ body: { padding: token.paddingSM } }}>
                                    <Space direction="vertical" size={2}>
                                      <Text type="secondary">Tổng kiện</Text>
                                      <Text strong>{quantity(rowParcels.length)}</Text>
                                    </Space>
                                  </Card>
                                </Col>
                                <Col xs={12}>
                                  <Card size="small" styles={{ body: { padding: token.paddingSM } }}>
                                    <Space direction="vertical" size={2}>
                                      ...
                                      <Text strong>{dateTime(row.createdAt)}</Text>
                                    </Space>
                                  </Card>
                                </Col>
                              </Row>
                              ...
                              {expanded && (
                                <Space direction="vertical" size={token.marginSM} style={styles.fullWidth}>
                                  {sortedParcels.length === 0 ? (
                                    ...
                                    sortedParcels.map((parcel, parcelIndex) => {
                                      const found = asArray(parcelStatuses).find(
                                        (item) => item.code === parcel.status,
                                      );

                                      return (
                                        <Card
                                          key={parcel.code ?? `parcel-${parcelIndex}`}
                                          size="small"
                                        ...
                                        >
                                          <Space direction="vertical" size={token.marginSM} style={styles.fullWidth}>
                                            <Flex justify="space-between" align="flex-start" gap={token.marginSM}>
                                              <Space direction="vertical" size={2}>
                                                <Text type="secondary">Mã kiện</Text>
                                            ...
                                            <Row gutter={[token.marginSM, token.marginSM]}>
                                              <Col xs={12}>
                                                <InfoBlock
                                                  ...
                                                  value={
                                                    parcel.volumetric
                                                      ? `${quantity(parcel.volumetric)} cm3`
                                                      : "Không xác định"
                                                  }
                                                />
                                              </Col>
                                              <Col xs={12}>
                                                <InfoBlock
                                                  label="Cân nặng"
                                                  value={
                                                    Number.isFinite(Number(parcel.actualWeight))
                                                      ? `${quantity(parcel.actualWeight)} kg`
                                                      : "Không xác định"
                                                  }
                                                />
                                              </Col>
                                                ...
                                                <InfoBlock
                                                  ...
                                                  value={
                                                    <Space direction="vertical" size={2}>
                                                      <span>
                                                        Chiều dài:{" "}
                                                        {Number.isFinite(Number(parcel.length))
                                                          ? `${parcel.length} cm`
                                                          : empty}
                                                      </span>
                                                      <span>
                                                        Chiều rộng:{" "}
                                                        {Number.isFinite(Number(parcel.width))
                                                          ? `${parcel.width} cm`
                                                          : empty}
                                                      </span>
                                                      <span>
                                                        Chiều cao:{" "}
                                                        {Number.isFinite(Number(parcel.height))
                                                          ? `${parcel.height} cm`
                                                          : empty}
                                                      </span>
                                                    </Space>
                                              ...
                                              <Col xs={12}>
                                                <Space direction="vertical" size={2}>
                                                  ...
                                                  <Tag color={found?.color || "default"}>
                                                    {display(found?.name ?? parcel.status)}
                                                  </Tag>
                                                </Space>
                                              </Col>
                                              <Col xs={12}>
                                                <Space direction="vertical" size={2}>
                                                  <Text type="secondary">Thời gian cập nhật</Text>
                                                  <Text strong>{shortDateTime(parcel.modifiedAt)}</Text>
                                                </Space>
                                              </Col>
                                            </Row>
                                          </Space>
              ...
              children: (
                <Spin spinning={isFinancialLoading}>
                  {asArray(financial).length > 0 ? (
                    <Space direction="vertical" size={token.marginSM} style={styles.fullWidth}>
                      {asArray(financial).map((record, index) => (
                        <Card
                          key={record.id ?? record.txid ?? `finance-${index}`}
                          size="small"
                          style={{
                            borderRadius: 0,
                            background: token.colorFillAlter,
                        ...
                        >
                          <Space direction="vertical" size={token.marginSM} style={styles.fullWidth}>
                            <Flex justify="space-between" align="flex-start" gap={token.marginSM}>
                              <Space direction="vertical" size={2} style={{ minWidth: 0 }}>
                                <Text type="secondary">{t("financial_tab.transaction_type")}</Text>
                                <Text strong style={{ wordBreak: "break-word" }}>
                                  {display(record.type?.name ?? record.type)}
                                </Text>
                              </Space>
                              <Text
                                strong
                            ...
                            <Row gutter={[token.marginSM, token.marginSM]}>
                              <Col xs={24} md={8}>
                                <Card size="small" styles={{ body: { padding: token.paddingSM } }}>
                                  <Space direction="vertical" size={2}>
                                    <Text type="secondary">{t("financial_tab.time")}</Text>
                                    <Text strong>{dateTime(record.timestamp)}</Text>
                                  </Space>
                                </Card>
                              </Col>
                              <Col xs={24} md={16}>
                                <Card size="small" styles={{ body: { padding: token.paddingSM } }}>
                                  <Space direction="vertical" size={2} style={styles.fullWidth}>
                                    <Text type="secondary">{t("financial_tab.content")}</Text>
